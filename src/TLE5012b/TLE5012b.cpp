...
TLE5012b::TLE5012b() : reg(this) {
    //this->spic = new SPIC(PIN_SPI_SCK, PIN_SPI_MISO, PIN_SPI_MOSI, PIN_SPI_SS);
    //this->spic->init();
    safetyWord = 0;
    mSlave = TLE5012B_S0;
}

errorTypes TLE5012b::begin(int8_t pinCS, SPIClass *spiClass) {
    this->spic = new SPIC(PIN_SPI_SCK, PIN_SPI_MISO, PIN_SPI_MOSI, pinCS, spiClass);
    this->spic->init();
    // start sensor
    enableSensor();
...
errorTypes TLE5012b::readBlockCRC() {
    ...
    _registers[CRC_NUM_REGISTERS + 1] = {0};  // Number of CRC Registers + 1 Reg for Safety word
    this->spic->sendReceiveSpi(_command, 1, _registers, CRC_NUM_REGISTERS+1);
...
errorTypes TLE5012b::readFromSensor(uint16_t command, uint16_t &data, updTypes upd, safetyTypes safe)
{
    ...
    this->spic->sendReceiveSpi(_command, 1, _received, 2);
    data = _received[0];
    if (safe == SAFE_high)
    {
...
errorTypes TLE5012b::readMoreRegisters(uint16_t command, uint16_t data[], updTypes upd, safetyTypes safe)
{
    ...
    this->spic->sendReceiveSpi(_command, 1, _received, _recDataLength + safe);
...
void TLE5012b::resetSafety() {
    ...
    uint16_t receive[4];
    this->spic->triggerUpdate();
    this->spic->sendReceiveSpi(&command, 1, receive, 3);
}
...
errorTypes TLE5012b::writeToSensor(uint16_t command, uint16_t dataToWrite, bool changeCRC)
{
    ...
    this->spic->sendReceiveSpi(_command, 2, &safety, 1);
